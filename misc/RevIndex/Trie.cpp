...
void Trie::doPutKey(std::string key, int pos, int64_t val, Node* head) {
    if (pos == key.length()) {
        return;
    }
    ...
    if (pos == key.length() - 1) {
        tmp->isKey = true;
        if (tmp->value_vec == nullptr) {
            tmp->value_vec = new std::vector<uint64_t>();
        }
        tmp->value_vec->push_back(val);// = val;
        numKeys++;
...
void Trie::putKey(std::string key, uint64_t val) {
...
std::vector<uint64_t> Trie::doGetKey(std::string key, int pos, Node * head) {
    //*val = -1;
    ...
    if (pos == key.length() - 1) {
        ...
        if (itr->second->isKey) {
            return *(itr->second->value_vec);
        }
        return vec;
    }
    
    return doGetKey(key, pos + 1, itr->second);
    
}

std::vector<uint64_t> Trie::getKey(std::string key) {
    return doGetKey(key, 0, root);
}
...
Node* Trie::doDeleteKey(std::string key, int pos, Node * head) {
    ...
    if (pos == key.length()) {
        head->isKey = false;
        delete head->value_vec;
        head->value_vec = nullptr;
        numKeys--;
    } else {
        char c = key[pos];
...
bool Trie::contains(std::string key) {
    std::vector<uint64_t> vec = doGetKey(key, 0, root);
    return !vec.empty();
}
